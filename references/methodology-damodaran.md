# Methodology: Damodaran's "Story 鈫?Numbers 鈫?Value"
...
> valuation.** See 搂6 "Data freshness protocol" below.
...
5. The decision layer (value 鈫?action)
...
| 2 | **Company narrative** | The firm's history, business model, competitive position, and the *risks embedded in the model*. | How does it actually make money? What is the moat? What structural risk comes with the model (e.g., NVIDIA's fabless reliance on TSMC 鈫?margin upside but Taiwan supply-chain risk)? |
| 3 | **Thematic / disruption** | The big driver. Is the change *revolutionary or incremental*? Break the ecosystem into layers and locate the company. | Where in the value chain does the company sit (hardware / software / data / applications)? How big is the addressable market and how fast does it grow? Does "everyone has it 鈫?no one does" erode the margin? |
| 4 | **Story 鈫?numbers (DCF)** | Translate the narrative into the four value drivers and run an intrinsic DCF. | What revenue, margin, reinvestment efficiency, and risk does the story imply? |
...
Damodaran reduces intrinsic value to four inputs. Everything in layers 1鈥? exists
to justify these four numbers:

1. **Revenue growth** 鈥?the proxy for growth potential. Built **bottom-up by
   segment** as *Total Addressable Market 脳 market share*. Growth declines over
   the horizon toward the riskfree rate.
2. **Target operating margin** 鈥?the proxy for profitability. Use the
   **R&D-adjusted** margin (see signature moves). Justify against the firm's
   history and peer group.
3. **Reinvestment efficiency** 鈥?measured by **sales-to-capital** ($ of revenue
   per $ of capital invested). Reinvestment = 螖Revenue 梅 sales-to-capital. Ground
   in the firm's history and industry median.
4. **Risk** 鈥?the **cost of capital** plus an optional **failure probability**.
   The cost of capital = riskfree rate + 尾 脳 ERP + country/geography adjustments.
   See `input-estimation.md` 搂4 for the full estimation protocol including
   the current implied ERP approach, country risk premiums, and the optional
   trust-deficit / institutional-risk overlay.

FCFF = After-tax operating income 鈭?Reinvestment. Discount at the cost of
capital; add a terminal value at stable growth (鈮?riskfree rate). See
...
- **Narrative emphasis** (what the story is about 鈥?market size vs. moat defense
  vs. capital return)
...
- **The financing and dividend assumptions** (all equity 鈫?debt capacity 鈫?
  returning cash 鈫?paying down debt)

The six phases are:

| Phase | Growth profile | Narrative emphasis | Valuation priority |
|-------|---------------|-------------------|-------------------|
| Start-Up | Zero revenue; idea stage | Can this idea become a business? | TAM 脳 probability of success; cash runway |
...
| High Growth | 30%+ revenue growth; margins turning positive | How big can this get? | TAM 脳 share; growth deceleration; margin expansion |
| Mature Growth | 5鈥?0% growth; positive margins expanding | How long can the moat defend above-average returns? | Competitive advantage period; margin durability |
| Mature Stable | 0鈥?% growth; stable margins; strong FCF | How will value be preserved? | Terminal value; capital allocation; payout |
...
- **Capitalize R&D.** Treating R&D as an operating expense understates margins
  ...
  pharma). Apply the same to leases. 鈫?*adjust the financials before you forecast
  them.*
- **Build revenue bottom-up: TAM 脳 share.** Don't extrapolate a single growth
...
- **Converge to the mean.** High margins, low cost of capital, and reinvestment
  ...
  returns fade unless a moat justifies otherwise. Terminal growth 鈮?riskfree rate
  (a company cannot outgrow the economy forever).
- **Quantify uncertainty; don't hide behind it.** "Too much uncertainty" is not a
  reason to skip the estimate 鈥?it's the reason to *simulate* it. Monte Carlo over
  the drivers 鈫?a value distribution. Breakeven table 鈫?"what has to be true".
...
- **Use the implied ERP, not a historical average.** Damodaran's forward-looking
  ...
  misrepresent the current risk environment. See 搂6 below.
...
- **Investing vs trading.** Investing closes the gap between price and *value*;
  ...
  agnostic to value. Decide which game you're playing 鈥?they need different tools.
- **Act on the gap, but allow pragmatism.** Value < price 鈫?sell discipline.
  Damodaran sold *half* and kept half for optionality 鈥?purist logic, pragmatic
...
1. Getting the macro story right 鈮?making money 鈥?you must pick the right company.
...
3. Even great companies have near-death drawdowns (NVIDIA fell >50% in 2002, 2008,
   2018) 鈥?which is when value investors get their entry.
...
| **ERP (historical)** | Annually | `pc/datasets/histretSP.xls` | For reference/comparison only. The historical average (1928鈥?024: ~5.5%) is NOT recommended as the primary ERP input 鈥?use the implied ERP. |
...
1. **Fetch the latest implied ERP** from Damodaran's data page. The current value
   ...
   with a link to `histimpl.xls`. web fetch: the HTML view at
...
4. **Use the current riskfree rate** from the market (US 10-year Treasury yield
   ...
   = 4.24% (T-bond) 鈭?0.27% (Aa1 default spread) = 3.97%.
...
3. Subtract the riskfree rate 鈥?the remainder is the implied ERP.
...
- Long-run historical ERP (1928鈥?024): 5.5%
...
0.25鈥?.00% to the cost of capital for companies whose operations or valuation
...
> The trust-deficit overlay is NOT a permanent fixture 鈥?it should be evaluated
...
| Layer / driver | Script |
|---|---|
| Story 鈫?numbers DCF (drivers 1鈥?) | `scripts/dcf_valuation.py` |
| Life-cycle phase calibration | `skills/classify-archetype/SKILL.md` 鈫?phase informs driver ranges, convergence timing, and terminal structure |
| Uncertainty 鈫?distribution | `scripts/monte_carlo.py` |
...
**Reference valuation ranges** (illustrative, from Damodaran's NVIDIA work 鈥?NOT
...
- The worked NVIDIA example (`templates/assumptions.example.json`) reproduces
  ...
  *numbers* in that example date to June 2023 鈥?the *method* is timeless.

---

## Related methodology documents

- `methodology-lifecycle.md` 鈥?The 6-phase corporate life cycle framework (2024 book)
- `input-estimation.md` 鈥?How to estimate each value driver, with current ERP protocol
- `analyst-playbook.md` 鈥?How top-tier research operationalizes each lens
- `valuation-lenses.md` 鈥?The four valuation lenses and triangulation rules
