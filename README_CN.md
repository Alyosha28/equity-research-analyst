...
| 模块 | 说明 |
|------|------|
| **估值引擎**（`scripts/`）| 10 个 Python 程序 —— DCF、蒙特卡洛、逆向 DCF、盈亏平衡、分部估值、可比分析、金融股估值、图表、报告质检、数据获取 |
| **方法论**（`references/`）| Damodaran 六层框架 + 输入估算手册 + 七视角交叉验证 + 分析师操作手册 |
| **公司类型手册** | 按行业/阶段分类 —— 成长/成熟型、银行/保险、周期/大宗商品、亏损/成长期、成熟/衰退型、控股公司 |
| **模板**（`templates/`）| 已跑通的完整案例 —— NVIDIA（2023.6，$240 内在价值 vs $409 市价）、腾讯 SOTP、Freeport-McMoRan |
| **测试**（`tests/`）| 可复现性守卫、输出格式校验、报告质检、公司类型覆盖 |
| **写作规范** | 投资者报告写作纪律 —— 禁止第二人称、禁止 emoji、强制安全边际区间、强制数字台账 |
...
| `/generate-pdf` | 生成排版级 PDF | 图表是否嵌入？中文字体是否渲染？评级框是否醒目？ |
...
| `charts.py` | 可视化 | 蒙特卡洛直方图、橄榄球场图、盈亏平衡热力图、龙卷风图 → PNG/SVG |
| `report_lint.py` | 质检关卡 | 检测 AI 痕迹：第二人称、emoji、缺失安全边际/台账/免责声明 |
...
| 类型 | 引擎 | 核心思路 | 示例 |
|------|------|----------|------|
| **成长/成熟型**（默认）| `dcf_valuation.py` | FCFF，四大驱动因子 | NVIDIA，多数实业公司 |
| **金融股**（银行、保险）| `financial_valuation.py` | 超额权益回报 `(ROE − Ke) × BV` | 大型银行、产险公司 |
| **周期/大宗商品** | `dcf_valuation.py` | 基于**标准化**周期中值的 FCFF + 商品价格中枢 | Freeport-McMoRan、石油公司 |
