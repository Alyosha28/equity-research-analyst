...
3. **Loop Protocol — Type-A/B gates + adversarial review.** Every gate is
   classified Type-A (machine-checkable: exit code, math, counter) or Type-B
   (requires taste/judgment). Type-B gates use adversarial review agents — currently
   same-model, with cross-model (Codex xhigh) as the target architecture. The
   five same-family-safe conditions and their compliance status are maintained in
   `references/loop-protocol.md` §6. Honest gap disclosure: the pipeline does NOT
...
├── /triangulate                  ← cross-check 8 lenses, identify dispute locus
...
## Shared resources
- `scripts/` — Python valuation engine (17 programs: dcf, mc, reverse_dcf, breakeven, comps, sotp, financial_valuation, fetch_financials, fetch_consensus, report_lint, financial_stress, verdict, build_manifest, build_industry_memory, management_credibility, select_charts)
- `skills/generate-charts/scripts/` — Chart rendering system (13 chart modules + theme + CJK + charts CLI). Story-driven chart selection via `scripts/select_charts.py`.
...
- Project entry point: `src/common/build_report.py` — reproducible PDF render (WeasyPrint default, logo auto-detect, config-driven). Chart contract: `![](figs/<ticker>_<kind>)`.
...
└──────────────────────────────────────────────────────────────────┘
                              ↓ (passes review)
WAVE 5 ─── spawn 3 agents in PARALLEL ─────────────────────────────┐
│  Agent F: /durability-check (ROIC-WACC, CAP, RONIC, moat)        │
│  Agent G: /triangulate      (8-lens cross-check, dispute locus)  │
│  Agent H: /generate-charts  (story-driven charts from engine     │
│           JSON; runs select_charts.py first, renders all charts   │
│           whose source data is ready, defers roic_spread until   │
│           durability.json exists)                                 │
│  → each followed by its adversarial reviewer                     │
└──────────────────────────────────────────────────────────────────┘
                              ↓ (all pass review)
                              ┌─── CONSISTENCY CHECK ───┐
...
### Phase 6: Analysis & Charts (WAVE 5 — parallel)
...
Agent G: /triangulate
  → adversarial review: all 8 lenses present? dispute locus specific? variant
    perception falsifiable? not averaging lenses?
  → if REVISE: fix → re-review

Agent H: /generate-charts (runs IN PARALLEL with durability + triangulation —
  charts only consume engine JSON, not durability/triangulation outputs)
  → STEP 0: Run scripts/select_charts.py TICKER --data-dir data/ --json
    (reads classification → archetype rules → narrative overrides → checks data
    readiness; outputs manifest with required/optional/skipped/data_ready/data_missing)
  → Renders all REQUIRED + OPTIONAL charts whose source JSON is ready
  → Charts needing durability.json (roic_spread) are deferred — a quick follow-up
    render runs after durability passes
  → Chart contract: ![](figs/<ticker>_<kind>)
  → Output: figs/<ticker>/<ticker>_{kind}.{svg,png} + chart-index.json
  → CJK font auto-detection, theme from config (default: Goldman-style palette)
  → adversarial review:
...
Agent K: /generate-pdf
  → Canonical entry point: python src/common/build_report.py TICKER
    (reads config.py for engine/logo height/inline SVG — reproducible across
    sessions without needing to remember CLI flags)
  → Engine: WeasyPrint default (native vector SVG, ~5x smaller PDFs, CSS Paged
    Media page numbering). Auto-falls back to Playwright if WeasyPrint unavailable.
  → Logo: auto-detected from assets/company_logo.* (when present). Top margin
    auto-grows to logo_height + 1.4cm so enlarged logos never overlap body text.
    Logo is optional — absent logo renders cleanly without branding.
  → Charts: all markdown chart refs MUST use ![](figs/<ticker>_<kind>) format.
    Missing figs/ prefix → chart unrecognized → PRE-RENDER FAIL.
  → --auto-charts flag runs select_charts internally for chart coverage
    verification (required charts missing → BLOCK before render)
  → CJK fonts auto-detected; rating box extracted and styled
  → Output: TICKER_research_report.pdf + preview PNGs (verify_visual=True)
  → adversarial review: all charts embedded? CJK fonts render? rating box
...
- **Story-driven chart selection:** The ticker archetype + narrative determine
  which charts to generate — NOT a fixed template. Run
